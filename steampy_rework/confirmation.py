...
from steampy_rework.exceptions import ConfirmationExpected
from steampy_rework.login import InvalidCredentials
...
class ConfirmationExecutor:
    ...
    def _send_confirmation(self, confirmation: Confirmation) -> dict:
        ...
        return self._session.get(self.CONF_URL + '/ajaxop', params=params, headers=headers, proxies=self._proxy).json()
    ...
    def _fetch_confirmations_page(self) -> requests.Response:
        ...
        response = self._session.get(self.CONF_URL + '/getlist', params=params, headers=headers, proxies=self._proxy)
    ...
    def _fetch_confirmation_details_page(self, confirmation: Confirmation) -> str:
        ...
        params = self._create_confirmation_params(tag)
        response = self._session.get(self.CONF_URL + '/details/' + confirmation.data_confid, params=params, proxies=self._proxy)
