import bs4
import re
from steampy_rework.models import Endpoints, SteamUrl
from steampy_rework.utils import account_id_to_steam_id
...
class SteamChat:
    ...
    def _get_access_token(self):
        response = self._session.get(SteamUrl.COMMUNITY_URL + "/chat", proxies = self.proxy )
    ...
    def _api_call(self, endpoint, params, timeout_ignore=False):
        response = self._session.post(endpoint, data=params, proxies = self.proxy)
        response.raise_for_status()
