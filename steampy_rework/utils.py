...
from steampy_rework.models import GameOptions
...
def merge_items_with_descriptions_from_inventory(inventory_response: dict, game: GameOptions) -> dict:
    ...
    return oldmerge_items(inventory, descriptions, context_id=game.context_id)


def oldmerge_items(items: List[dict], descriptions: dict, **kwargs) -> dict:
    merged_items = {}
    for item in items:
...
def get_market_listings_from_html_for_confirm(html: str) -> dict:
    ...
    for node in nodes:
        if "My sell listings" in node.text:
            sell_listings_dict = get_sell_listings_from_node_for_confirm(node)
        elif "My listings awaiting confirmation" in node.text:
            sell_listings_awaiting_conf = get_sell_listings_from_node_for_confirm(node)
    ...
    return {"sell_listings_for_confirm": sell_listings_dict}
...
def get_sell_listings_from_node_for_confirm(node: Tag) -> dict:
    sell_listings_raw = node.findAll("div", {"id": re.compile('mylisting_\d+')})
    sell_listings_dict = {}
    for listing_raw in sell_listings_raw:
        data_asset_id = listing_raw.find_all("a", {"class": "item_market_action_button item_market_action_button_edit nodisable"})
        asset_id_data = (str(data_asset_id).strip()[1:-1].split("', '")[2].split("')\">")[0])
        spans = listing_raw.select("span[title]")
        dataListing_id = listing_raw.attrs["id"].replace("mylisting_", "")
        name = listing_raw.findAll("span",{"id": f"mylisting_{dataListing_id}_name"})
        name = str(name).split('">')[2].split('</a></span>')[0]
        listing = {
            "name": name,
            "asset_id": asset_id_data,
            "listing_id": dataListing_id,
...
def get_description_key_from_offer(item: dict) -> str:
    return item['classid'] + '_' + item['instanceid']


def get_name_and_descriptions_key_form_descriptions(response: dict) -> dict: #Получаю предметы и их айди из описания
    """
    return dict with keys classid_instanceid and value market_hash_name
    """
    if 'descriptions' in response['response']:
        data_items = {}
...
def get_trade_offers_data(response:dict):
